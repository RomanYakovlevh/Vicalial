...
export class MatrixReplace extends NamedMatrix {
    ...
    constructor(matrixLeft: Matrix, matrixRight: Matrix, selectionFromLeft: Array<{ row: number, col: number }>, selectionFromRight: Array<{ row: number, col: number }>) {
        super(replace(matrixLeft, selectionFromLeft, matrixRight, selectionFromRight))
    ...
    getRelative() {
        return this.parentLeft.getName() + "" + this.selectionFromLeftDescription + " replaced with " + this.parentRight.getName() + "" + this.selectionFromRightDescription
    }
